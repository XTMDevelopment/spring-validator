...
/**
 * Test to verify all English message codes are properly defined and resolvable.
 */
class EnglishMessageCodesTest {

    @Test
    void shouldLoadAllEnglishMessages() throws IOException {
    ...
    @Test
    void shouldResolveAllEnglishMessages() throws IOException {
    ...
    void shouldFormatMessagesCorrectly() {
        ...
        assertThat(message1).isEqualTo("email is required");

        String message2 = resolver.getMessage("validation.spring.size", "password", 8, 20);
        assertThat(message2).isEqualTo("password must be between 8 and 20 characters");

        String message3 = resolver.getMessage("validation.common.in-whitelist", "status", "active,inactive");
        assertThat(message3).isEqualTo("status must be one of: active,inactive");

        String message4 = resolver.getMessage("validation.data.tax-id.length", "taxId", "15-16");
        assertThat(message4).isEqualTo("taxId must have 15-16 digits");

        String message5 = resolver.getMessage("validation.datetime.invalid-past-date", "date");
        assertThat(message5).isEqualTo("date must be a date equals today");

        String message6 = resolver.getMessage("validation.datetime.invalid-future-date", "date");
        assertThat(message6).isEqualTo("date must be a date equals today");
    }
